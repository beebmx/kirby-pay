...
<div class="kirby-pay">
    <form class="<?= kpStyle('form', 'kp-form') ?>"
          x-data='{...order(), ...(new KirbyPay("<?= kpUrl("order.create") ?>","<?= kpMethod("order.create") ?>","<?= substr(kirby()->language()->code(), 0, 2) ?>")).order({id:"<?= $uuid ?? null ?>",items:<?= json_encode($items ?? []) ?>,<?php if(kpHasShipping()): ?>shipping:<?= json_encode($shipping ?? []) ?>,<?php endif ?>country:"<?= pay('default_country') ?>"})}'
          x-init="mount"
